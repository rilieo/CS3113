...
Map::Map(int width, int height, signed int *level_data, GLuint texture_id, float tile_size, int tile_count_x, int tile_count_y)
{
    m_width = width;
    m_height = height;
...
bool Map::is_solid(glm::vec3 position, float *penetration_x, float *penetration_y)
{
    ...
    *penetration_y = 0;
    ...
    if (tile == -1 || tile == 1 || tile == 0) return false;
...
void Map::set_current_tile(glm::vec3 position, int new_tile) {
    int tile_x = floor((position.x + (m_tile_size / 2)) / m_tile_size);
    int tile_y = -(ceil(position.y - (m_tile_size / 2))) / m_tile_size; // Our array counts up as Y goes down.
    
    m_level_data[tile_y * m_width + tile_x] = new_tile;
}

std::pair<int, int> const Map::get_current_tile(glm::vec3 position) {
    int tile_x = floor((position.x + (m_tile_size / 2)) / m_tile_size);
    int tile_y = -(ceil(position.y - (m_tile_size / 2))) / m_tile_size; // Our array counts up as Y goes down.
