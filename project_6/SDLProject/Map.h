...
public:
    Map(int width, int height, signed int *level_data, GLuint texture_id, float tile_size, int
    tile_count_x, int tile_count_y);
    
    void build();
    void render(ShaderProgram *program);
    bool is_solid(glm::vec3 position, float *penetration_x, float *penetration_y);
    void set_current_tile(glm::vec3 position, int tile);
    std::pair<int, int> const get_current_tile(glm::vec3 position);
    
    std::set<std::pair<int, int>> m_planted;
    
    // Getters
    int const get_width()  const  { return this->m_width;  }
    int const get_height() const  { return this->m_height; }
    
    signed int* const get_level_data() const { return this->m_level_data; }
