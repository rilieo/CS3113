...
bool const Map::is_on_ladder(glm::vec3 position) {
    std::set<int> ladder_tiles = {0, 1, 2, 3, 4, 5};
    
    int tile = get_current_tile(position);
    
    return ladder_tiles.find(tile) != ladder_tiles.end();
}

bool const Map::get_current_tile(glm::vec3 position) {
    int tile_x = floor((position.x + (m_tile_size / 2)) / m_tile_size);
    int tile_y = -(ceil(position.y - (m_tile_size / 2))) / m_tile_size; // Our array counts up as Y goes down.
